...
SpriteMorph.prototype.initBlocks = function () {
    SpriteMorph.prototype.blocks = {
        ...
        talkToMotor: {
            type: 'command',
            category: 'motion',
            spec: 'talk to motor %s',
            defaults: ['a']
        },
        turnMotor: {
            type: 'command',
            category: 'motion',
            spec: 'turn motor %onoff',
            defaults: ['On']
        },
        motorCW: {
            type: 'command',
            category: 'motion',
            spec: 'motor direction %clockwise'
        },
        motorCCW: {
            type: 'command',
            category: 'motion',
            spec: 'motor direction %counterclockwise'
        },
        setPower: {
            type: 'command',
            category: 'motion',
            spec: 'set motor power %n',
            defaults: [30]
        },
        forward: {
            type: 'command',
...
SpriteMorph.prototype.blockTemplates = function (category) {
    ...
    if (cat === 'motion') {
				...
				//stitchzaa
				blocks.push(block('beep'));
				blocks.push(block('turnLED'));
				blocks.push(block('talkToMotor'));
				blocks.push(block('turnMotor'));
				blocks.push(block('motorCW'));
				blocks.push(block('motorCCW'));
				blocks.push(block('setPower'));
				blocks.push('-');
    ...
    */
        ...
        blocks.push('-'); /// stitchzaa
        blocks.push(watcherToggle('sensor1'));
        blocks.push(block('sensor1'));
        blocks.push(watcherToggle('sensor2'));
        blocks.push(block('sensor2'));
        blocks.push(watcherToggle('sensor3'));
        blocks.push(block('sensor3'));
        blocks.push(watcherToggle('sensor4'));
        blocks.push(block('sensor4'));
        blocks.push(watcherToggle('sensor5'));
        blocks.push(block('sensor5'));
        blocks.push(watcherToggle('sensor6'));
        blocks.push(block('sensor6'));
        blocks.push(watcherToggle('sensor7'));
        blocks.push(block('sensor7'));
        blocks.push(watcherToggle('sensor8'));
        blocks.push(block('sensor8'));
        blocks.push('-');
...
SpriteMorph.prototype.beep = function () {
    if (window.GB) {
    	window.GB.talkToGogo('beep');
    }
};

SpriteMorph.prototype.turnLED = function (data) {
    if (window.GB) {
    	window.GB.talkToGogo('led'+data);
    }
};

SpriteMorph.prototype.talkToMotor = function (data) {
    if (window.GB) {
    	data = data.toLowerCase();
    	var motor = ((data.indexOf('a') >= 0) ? 1 : 0) + ((data.indexOf('b') >= 0) ? 2 : 0) + ((data.indexOf('c') >= 0) ? 4 : 0) + ((data.indexOf('d') >= 0) ? 8 : 0);
    	window.GB.talkToGogo('talkToMotor::'+motor);
    }
};

SpriteMorph.prototype.turnMotor = function (data) {
    if (window.GB) {
    	window.GB.talkToGogo('motor'+data);
    }
};

SpriteMorph.prototype.motorCW = function () {
    if (window.GB) {
    	window.GB.talkToGogo('motorCW');
    }
};

SpriteMorph.prototype.motorCCW = function () {
    if (window.GB) {
    	window.GB.talkToGogo('motorCCW');
    }
};

SpriteMorph.prototype.setPower = function (data) {
    if (window.GB) {
    	window.GB.talkToGogo('setPower::'+data);
    }
};
...
// Extra code from stitchzaa


SpriteMorph.prototype.sensor1 = function () {
    return window.sensor ? window.sensor[0] : 0;
};

SpriteMorph.prototype.sensor2 = function () {
    return window.sensor ? window.sensor[1] : 0;
};

SpriteMorph.prototype.sensor3 = function () {
    return window.sensor ? window.sensor[2] : 0;
};

SpriteMorph.prototype.sensor4 = function () {
    return window.sensor ? window.sensor[3] : 0;
};

SpriteMorph.prototype.sensor5 = function () {
    return window.sensor ? window.sensor[4] : 0;
};

SpriteMorph.prototype.sensor6 = function () {
    return window.sensor ? window.sensor[5] : 0;
};

SpriteMorph.prototype.sensor7 = function () {
    return window.sensor ? window.sensor[6] : 0;
};

SpriteMorph.prototype.sensor8 = function () {
    return window.sensor ? window.sensor[7] : 0;
};
